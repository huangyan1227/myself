...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem= [[UIBarButtonItem alloc]initWithTitle:@"退出" style:UIBarButtonItemStylePlain target:self action:@selector(backtui)];
    //设置扫码后需要扫码图像
...
//        NSString *a  =[NSString stringWithFormat:@"%@",result];
//        NSURL * url = [NSURL URLWithString: a];
//        if ([[UIApplication sharedApplication] canOpenURL: url]) {
//            [[UIApplication sharedApplication] openURL: url];
//        } else {
//            UIAlertView * alertView = [[UIAlertView alloc] initWithTitle: @"警告" message: [NSString stringWithFormat: @"%@:%@", @"无法解析的二维码", result] delegate: nil cancelButtonTitle: @"确定" otherButtonTitles: nil];
//            [alertView show];
//        }
        

        NSLog(@"qqscanResult:%@",result.strBarCodeType);
    }
...
//    [self.navigationController pushViewController:vc animated:YES];
    ...
    ScienceViewController * vc = [ScienceViewController new];
    
    vc.strSCan = strResult.strScanned;
    
    NSLog(@"gfda%@",strResult);
    [_scandelegate result:strResult.strScanned];
    [self dismissViewControllerAnimated:YES completion:nil];
}
